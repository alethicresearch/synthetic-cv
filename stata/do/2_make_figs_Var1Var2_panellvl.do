...
*******************************************************************************
********************************************************************************
* Study 1 WTP estimation — PANEL-LEVEL version
*   No collapse to caseid. Each persona × LLM × repetition is its own row.
*   Produces well-behaved logits with binary outcomes (no fractional means).
*   Standard errors clustered at caseid where supported.
*
* Data: ${datadir}/working_data.dta
* Output files (same names as before, will overwrite):
...
*
* CAVEAT: wtpcikr's Krinsky-Robb bootstrap samples from the asymptotic
* distribution of e(b)/e(V). With panel-level data the effective N is
* persona × LLM × repetitions, so e(V) is too small and the resulting
* CIs are anti-conservative. The point estimate is unbiased.
...
********************************************************************************
* BLOCK 1 — Empirical replication (unchanged)
********************************************************************************
...
********************************************************************************
* BLOCK 2 — Demographics-only synthetic (panel-level, no collapse)
********************************************************************************
	...
	* ─── Per-LLM ──────────────────────────────────────────────────────────────
	forvalues p = 1/9 {
		...
		capture noisily wtpcikr ${aldy_rhs} if provider_id == `p', ///
			reps(1000) mym(temppool)
		if _rc != 0 {
...
********************************************************************************
* BLOCK 3 — Demographics + all beliefs synthetic (panel-level, no collapse)
********************************************************************************
	...
	* ─── Per-LLM ──────────────────────────────────────────────────────────────
	...
	di as result "All three WTP blocks complete (panel-level)."
