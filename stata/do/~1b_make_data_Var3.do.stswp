...
*******************************************************************************	
	...
	* Use the provider_id from the Var1Var2 data
	preserve
		use ${datadir}/working_data.dta, clear
		collapse (first) provider, by(provider_id)
		save ${datadir}/provider_id_xwalk.dta, replace
	restore
	merge m:1 provider using ${datadir}/provider_id_xwalk.dta
	drop _merge
*******************************************************************************
...
*******************************************************************************
	...
	* Making belif Q variable that is unique per question (regardless of response)
	// Just by tabbing "matched" for each of these, I can see that Q 3 and Q4 are best predictors--roughly 67% matched=true. Next best is Q5 with 64% matched=true
	// Also, I'm not sure why beliefQ_id is made up of two different questions that shuold actually just be one...
	gen beliefQ_id = .
