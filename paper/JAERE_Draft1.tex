...
Estimates of willingness to pay (WTP) for nonmarket goods are used in many decision-making contexts. The most prominent example in the United States is federal benefit-cost analysis, where WTP-derived values for mortality risk reductions, morbidity, environmental amenities, and ecosystem services inform regulatory impact analyses across agencies. But the applications extend well beyond federal rulemaking. Natural resource damage assessments under CERCLA (the Comprehensive Environmental Response, Compensation, and Liability Act), the Oil Pollution Act, and the Clean Water Act require monetizing injuries to public resources, and stated preference methods have played a central role in this context since the Exxon Valdez litigation. State and municipal governments draw on WTP estimates for air and water regulation, park and transit investments, and green infrastructure. International institutions—including the World Bank, OECD, and various UN bodies—rely on nonmarket valuation in project appraisal and in the damage functions underlying integrated assessment models of climate change. In the corporate sphere, impact-weighted accounting, natural capital accounting, and true cost accounting frameworks all monetize externalities using WTP-based methods. Health technology assessment bodies outside the US, such as the UK's National Institute for Health and Care Excellence (NICE) and Canada's Drug Agency (CAD), rely on WTP thresholds per quality-adjusted life year to guide coverage decisions. And conservation organizations and agencies use WTP estimates—including measures of existence and bequest value—to prioritize land acquisition, species recovery, and habitat restoration. 

The standard tool for estimating WTP for nonmarket goods is the contingent valuation (CV) method, which elicits WTP for hypothetical policy changes through structured surveys. CV studies, however, are slow and expensive. A nationally representative referendum-style survey with adequate statistical power for benefit-cost analysis typically requires \$[\textbf{XXX,XXX}] in funding and [\textbf{6--12}] months of design, sampling, and analysis time \parencite{Johnston2017, Champ2017}. Cost and timeline considerations can prevent stakeholders from investing in WTP estimates, and thus prevent important values from being represented in decision-making.

This paper investigates whether large language models (LLMs) can help address this problem. The basic idea, which has gained traction in marketing research \parencite{Brand2024, Li2024} and political science \parencite{Argyle2023, Bisbee2024}, is to substitute LLM-generated responses for human responses in a survey instrument. If this substitution is likely to produce results that are sufficiently close to the ones that would be produced by surveying people, then a ``synthetic'' study can be executed in hours rather than months at a fraction of the cost. Applied to CV, synthetic experiments could, in principle, allow analysts to produce ballpark WTP estimates for nonmarket goods that would otherwise be left unquantified, expanding the set of values that enter benefit-cost analysis at all. However, it remains to be seen whether substituting LLM-generated responses do, in fact, produce results that are sufficiently close to the ones that would be produced by surveying people.

We begin to address this question through synthetic replications of two published CV studies. \textcite{Aldy2012} estimates WTP for a U.S. National Clean Energy Standard (NCES) requiring 80\% clean electricity by 2035, using a nationally representative referendum-style survey of 1{,}010 respondents. \textcite{Giguere2020} estimates WTP for control of a pest species, the hemlock woolly adelgid (HWA), in North Carolina public forests, using an online panel of 907 respondents and both bounded and random-cost choice formats. The studies span substantively different policy domains, sample frames, and elicitation formats, providing a useful cross-context test of synthetic CV. For each study, we reconstruct synthetic personas from the original respondents' demographic and belief responses, prompt ten frontier and mid-tier LLMs to act as those respondents, and analyze the resulting synthetic responses using the original studies' econometric specifications. We then compare synthetic WTP estimates to the published human-survey benchmarks of \$162 per household per year (Aldy et al.) and \$56.53 per household per year (Giguere et al.).
...
Second, the failure is structured rather than random: the synthetic approval-bid curves are systematically flatter than empirical curves regardless of model, condition, or study. Because logit-implied WTP is the ratio of the constant to the negated bid coefficient, this slope compression mechanically inflates WTP whenever level fit is reasonable. The compression is the central failure of current synthetic CV, and we devote a substantial portion of the paper to characterizing it.

Third, the compression is identifiable at the token-probability layer, not introduced by stochastic sampling. Using next-token log-probabilities for the response tokens \texttt{Support} and \texttt{Oppose}, we show that the LLMs' implicit utility difference is attenuated relative to the empirical logit. A complementary flat-slope benchmark---a CV-like task in which the dollar amount is normatively irrelevant---establishes that the compression is not primarily a numerical-anchoring artifact. We use the flat-slope benchmark to construct per-model calibration ratios that, applied to the bid coefficient, recover synthetic WTP estimates within the original 95\% confidence intervals for [\textbf{X of 10}] models in Study 1 and [\textbf{X of 10}] models in Study 2.
...
\subsection{Implications for Practice}\label{sec:disc-policy}

We close by returning to the practical motivation that opened the paper. the costs and benefits relevant to many analyses include non-market values that are expensive and slow to estimate using conventional CV, and the consequence of neglecting these values in the analysis is decisions made without full consideration of relevant costs and benefits.

Our results do not support the use of synthetic CV as a drop-in substitute for conventional CV in real-world decision making. The slope-compression mechanism produces biased point estimates of WTP that, if used in net-benefit calculations, will systematically distort regulatory choices. We do, however, see three uses for synthetic CV in practice that our results support.

First, synthetic CV is well-suited to \emph{threshold analysis}, the practice of identifying break-even values for non-quantified factors. An analyst who can establish that stakeholders' WTP for a non-market good plausibly exceeds some lower bound---and can do so quickly using synthetic CV---is in a stronger position to argue that the good warrants consideration in the analysis even without a precise point estimate.

Second, synthetic CV is well-suited to \emph{sensitivity testing} of conventional benefit transfer. An analyst using a transferred WTP estimate from a published study can use synthetic CV with the target-context demographics to assess whether the transferred value is plausible given the local population's characteristics, and can flag cases where the transferred value appears systematically off.
...
We do not believe synthetic CV is ready to substitute for conventional CV in cases where decisions hinge on precise WTP point estimates. We do believe it is ready, today, to expand the set of values that can be considered in analysis when the alternative is leaving them unexamined. The history of stated preference methods suggests that the field's most productive moves have come from expanding the range of values that can be brought into benefit-cost analysis while maintaining methodological discipline about the resulting estimates' reliability. Synthetic CV, used carefully, fits squarely in that tradition.
